...
namespace FoodBridge.Server.Services
{
    ...
    .Include(i => i.SourceDonationItem)  // ? Include SourceDonationItem
    .ThenInclude(sdi => sdi.Product)    // ? Then include Product
       .Include(i => i.SourceDonationItem.Donation)
     .AsQueryable();
 ...
 query = query.ApplySearch(filter.SearchTerm,
      i => i.SourceDonationItem.Product.ProductName,  // ? Access through SourceDonationItem
       i => i.SourceDonationItem.Product.Category,
 ...
 try
 {
   var item = await _context.InventoryItems
    .Include(i => i.SourceDonationItem)
     .ThenInclude(sdi => sdi.Product)
       .Include(i => i.SourceDonationItem.Donation)
  ...
  i.ExpirationDate.Value <= expiryDate &&
   ...
   return items.Select(i => i.ToDto(_mapper)).ToList();
        ...
        public async Task<InventoryItemDto> CreateAsync(CreateInventoryItemDto dto)
        {
          try
   {
   // ? Validate: SourceDonationItemId must exist
           var donationItem = await _context.DonationItems
  .Include(di => di.Product)
      .FirstOrDefaultAsync(di => di.DonationItemId == dto.SourceDonationItemId);

          if (donationItem == null)
{
        throw new InvalidOperationException(
        $"Source donation item {dto.SourceDonationItemId} not found");
         }
...
try
            ...
            catch (Exception ex)
 {
  ...
  if (filter.ProductId.HasValue)
   query = query.Where(i => i.SourceDonationItem.ProductId == filter.ProductId.Value);  // ? Through SourceDonationItem

        if (!string.IsNullOrWhiteSpace(filter.Category))
        query = query.Where(i => i.SourceDonationItem.Product.Category == filter.Category);  // ? Through SourceDonationItem.Product
