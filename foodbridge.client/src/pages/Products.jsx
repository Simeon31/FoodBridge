...
const ProductsPage = () => {
    ...
    const handleView = async (product) => {
        try {
            ...
            setError(err, 'Failed to load product details');
        }
    };
    ...
    const columns = [
        ...
        { header: 'Unit', accessor: 'defaultUnitType' },
        {
            header: 'Perishable',
            render: (row) => (
                <span className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${row.isPerishable ? 'bg-warning text-warning' : 'bg-success text-success'
                    }`}>
        ...
        },
        {
            header: 'Status',
            render: (row) => (
                <span className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${row.isActive ? 'bg-success text-success' : 'bg-danger text-danger'
                    }`}>
                    ...
                    <div className="w-full max-w-2xl rounded-lg bg-white dark:bg-boxdark p-6 shadow-lg">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <span className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${selectedProduct.isActive ? 'bg-success text-success' : 'bg-danger text-danger'
                                    }`}>
